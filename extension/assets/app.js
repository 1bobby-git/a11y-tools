(function(){
  ...
  let report=null, toastTimer=null, previewChannel=null, previewReportId=null, previewRecording=false, scriptCache=null;
  ...
  function loadReport(value,navigate=true){
    report=validateReport(value);persist();renderAll();
  ...
  function renderFindings(){
    ...
    for(const f of list){
      ...
      body.append(meta,el('p',safeString(f.detail)),el('h3','대상 마크업 (입력값·URL 속성 제외)'),el('pre',safeString(f.html)),el('h3','수정 방향 / 예시'),el('pre',safeString(f.fix)||'실제 화면과 보조기술 사용 결과를 함께 확인하세요.'));
  ...
  function reportHTML(){
    if(!report)return '';
    const findings=report.findings.map(f=>`<article><h3>${esc(f.status==='error'?'자동 오류':'검토 필요')} · ${esc(f.title)}</h3><p>${esc(f.detail)}</p><pre>${esc(f.selector)}\n${esc(f.html)}</pre><p>${esc(f.fix)}</p></article>`).join('');
  ...
  async function getScripts(){if(!scriptCache)scriptCache=Promise.all(['core.js','focus.js','vendor-axe.js'].map(async file=>{const r=await fetch('assets/'+file);if(!r.ok)throw new Error('검사 엔진 파일을 읽지 못했습니다. 로컬 서버 또는 게시된 웹에서 실행하세요.');return r.text();})).catch(e=>{scriptCache=null;throw e;});return scriptCache;}
  ...
  async function runSource(source,mode='source'){
    ...
    const setup=`\n(async function(){const channel=${JSON.stringify(channel)};let busy=false;const send=(type,payload)=>parent.postMessage({channel,type,payload},'*');document.addEventListener('submit',e=>e.preventDefault(),true);document.addEventListener('click',e=>{const a=e.target.closest?.('a');if(a&&!String(a.getAttribute('href')).startsWith('#'))e.preventDefault();},true);window.addEventListener('message',e=>{if(e.source!==parent||e.data?.channel!==channel)return;const d=e.data;if(d.type==='start'){StudioFocus.start(()=>send('focus',StudioFocus.report()));send('started',null);}if(d.type==='stop'){send('focus',StudioFocus.stop());send('stopped',null);}if(d.type==='locate'){const el=StudioAudit.find(d.selector);if(el){el.scrollIntoView({block:'center'});send('located',d.selector);}}});try{const result=await StudioAudit.run(document,{mode:${JSON.stringify(mode)},state:'격리된 HTML 미리보기'});result.limitations.push(${JSON.stringify('원본 스크립트·외부 리소스를 차단한 HTML 검사입니다. 제거한 실행·외부 연결 항목 '+removed+'개. 실사이트와 레이아웃·동작이 다를 수 있습니다.')});send('report',result);}catch(e){send('error',String(e.message));}})();`;
  ...
  window.addEventListener('message',e=>{
    ...
    if(type==='report'){try{previewReportId=payload.id;loadReport(payload);}catch(err){toast(err.message);}}
    if(type==='error')toast('검사 실패: '+payload);
  ...
  $('clear-session').addEventListener('click',()=>{try{sessionStorage.removeItem('a11y-studio-report');}catch{}report=null;renderAll();location.hash='inspect';toast('이 페이지의 보고서를 지웠습니다.');});
  window.StudioApp=Object.freeze({loadReport,getReport:()=>report,validateReport,reportHTML,showView,runSource});
