...
(function(g){
'use strict';
if(g.StudioPlayer?.version==='0.3.1')return;
g.StudioPlayer?.stop();
...
function candidates(){
  ...
  function visit(n){
    ...
    if(n.nodeType===1){
      ...
      const s=getComputedStyle(n);if(s.display==='none')return;
      // A descendant can override inherited visibility without revealing its parent.
      ...
      if(n.matches('a[href],area[href],button,input:not([type="hidden"]),select,textarea,summary,[tabindex],audio[controls],video[controls],iframe')||editable){
        ...
        if(visible&&!n.matches(':disabled')&&t>=0&&summary&&n.getClientRects().length&&typeof n.focus==='function')ordered.push({node:n,tab:t,order:ordered.length});
      }
...
g.StudioPlayer=Object.freeze({version:'0.3.1',candidates,state,play,pause,stop,command,configure,subscribe:fn=>{notify=typeof fn==='function'?fn:()=>{};}});
})(globalThis);
